...
import { SelectListProps } from "../shared/SharedInterface";

type L1Keys = { key?: any; value?: any; disabled?: boolean | undefined };

const Dropdown: React.FC<SelectListProps> = ({
  txtValue,
  setSelected,
  placeholder,
  boxStyles,
  ...
  const [selectedval, setSelectedVal] = React.useState<any>(txtValue);
  const [height, setHeight] = React.useState<number>(200);
  const animatedvalue = React.useRef(new Animated.Value(0)).current;
  const [filtereddata, setFilteredData] = React.useState(data);

  useEffect(() => {
    setSelectedVal(txtValue);
  }, [txtValue]);
